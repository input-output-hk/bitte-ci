...
let
  inherit (builtins) attrNames fromJSON listToAttrs readFile toFile toJSON typeOf unsafeDiscardStringContext;
  ...
  pullRequests' = if typeOf pullRequests == "attrs" then pullRequests else importJSON pullRequests;
  ...
  mkPRJobset =
    prNumber: { title, head, ... }:
    nameValuePair "pr-${prNumber}" (mkJobset "refs/pull/${prNumber}/head" title);
...
in {
  jobsets = derivation {
    ...
    system = "x86_64-linux";
    builder = "/bin/sh";
    args = [ (toFile "spec-builder.sh" ''
      while read -r; do echo "$REPLY"; done <<EOF > $out
      ${toJSON jobsets}
      EOF
    '') ];
  };
